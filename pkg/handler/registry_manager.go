...
const (
	...
	registryStorageClass  = "lvm"
	registryStorageSize   = "50Gi"
	registryAdminPassword = "zcloud"
...
func (m *RegistryManager) Create(ctx *resttypes.Context, yaml []byte) (interface{}, *resttypes.APIError) {
	...
	requiredStorageClass := registryStorageClass
	if r.StorageClass != "" {
...
func (m *RegistryManager) get(ctx *resttypes.Context) *types.Registry {
	cluster := m.clusters.GetClusterForSubResource(ctx.Object)
	if cluster == nil {
		return nil
	}
	...
	r, err := genRegistryFromApp(ctx, cluster.Name, apps[0])
	if err != nil {
		return nil
	}
...
func genRegistryConfigs(cli client.Client, r *types.Registry, clusterName string) ([]byte, error) {
	if len(r.IngressDomain) == 0 {
		...
		r.IngressDomain = registryAppNamePrefix + "-" + ZCloudNamespace + "-" + clusterName + "." + firstEdgeNodeIP + "." + ZcloudDynamicaDomainPrefix
	}
	...
	harbor := charts.Harbor{
		...
		Persistence: charts.HarborPersistence{
			StorageClass: registryStorageClass,
			StorageSize:  registryStorageSize,
		},
		AdminPassword: registryAdminPassword,

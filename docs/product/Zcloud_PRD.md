...
* chart资源：chart资源提供chart的信息展示，配置展示等。在chart的详情页面可以进行app资源的安装。

* app资源：可以包含多个k8s资源。需要在指定的namespaces进行安装。app资源管理需要有单独的管理UI。
...
|          | **功能**     | **子功能（栏目）**           | **功能点描述** |
| -------- | ------------ | ---------------------------- | -------------- |
| 应用商店 | 1、chart资源 | 展示   <br/>安装             |                |
|          | 2、app资源   | 总览   <br/>卸载   <br/>详情 |                |

### **5.5.4**    子功能详细描述

#### **5.5.4.1**  展示

​		Zcloud启动时，在本地获取chart资源列表展示， 展示内容：icon，chart名称，描述，详情按钮。高级功能，支持云端列表对比并拉取最新的chart和chart的最新版本。

#### **5.5.4.2**  安装

* 若通过chart进行app资源的安装，需要根据app资源预设的属性进行设置后，再进行安装。
* 因我们规定，app资源的安装必须在指定的namespaces下进行，所以需要对chart进行检测，不允许chart进行namespaces的创建操作。

* 同一个chart在同一个namespaces下，可以进行多次安装，需要指定不同的k8s资源名称。

#### **5.5.4.3**  总览

对已安装的app资源进行展示，并且可以按chart进行分组，清晰的定义哪些app资源是从同一个chart进行安装的，这里不需要细化到版本，只细化到chart的名称即可。

* app资源展示的概要内容如下：icon，app资源名称，svc端口号，副本数；
* 可进行的操作如下：升级、降级、删除。暂不支持clone功能
...
chart version，创建时间，k8s相关资源，显示格式参照5.4的内容。

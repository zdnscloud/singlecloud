...
此功能为用户提供实时的流量展示功能。通过workload的出入流量，或是左侧菜单页进入此功能。支持的过滤条件workload类型，workload名称，流量的方向，HTTP方法，HTTP路径。其中workload类型，workload名称可以唯一标识一个资源，进行实时流量的展示。
...
以上动作的失败是分两部分，一个是zcloud失败，这个是立即返回的。一个是k8s失败，这个是异步发生的。所以我们只需要记录异步失败事件并注明原因。通过铃铛页面进行展示即可。立即返回的不在铃铛中记录。

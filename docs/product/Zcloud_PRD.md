...
* chart资源：********chart的定义****chart资源提供chart的信息展示，配置展示等。在chart的详情页面可以进行app资源的创建。
...
|      | **功能**     | **子功能**                           | **功能点描述** |
| ---- | ------------ | ------------------------------------ | -------------- |
|      | 1、chart资源 | 本地加载<br/>展示   <br/>创建app资源 |                |
|      | 2、app资源   | 获取   <br/>删除                     |                |
...
​		Zcloud启动时，在本地获取chart资源列表展示， 展示内容：icon，chart名称，描述，详情按钮。

#### **5.5.4.2**  创建app资源

* 若通过chart进行app资源的创建，需要根据app资源预设的属性进行设置后，再进行创建。
* 因我们规定，app资源的创建必须在指定的namespaces下进行，所以需要对chart进行检测，不允许chart进行namespaces的创建操作。

* 同一个chart在同一个namespaces下，可以进行多次创建，需要指定不同的k8s资源名称。
...
chart version，创建时间，k8s相关资源，显示格式参照5.4的内容。并且app引用资源在每一个分项workload里，也可以进行查看和编辑。

...
记录Zcloud平台发生的事件，事件分类如下：k8s事件、Zcloud事件、预警。

某一类型可以选择忽略，或者恢复。UI铃铛要显示总条数，忽略的类型不计数。

事件产生，目前的通知仅支持页面通知。将来用户接收具体类型可选择。
...
只显示node、cluster、namespaces级别的事件，workload与应用的事件不显示。  沿用现有的k8s事件展示

#### **4.6.3.2**      Zcloud事件

用户的操作记录，集群的创建、编辑、删除，节点的新增、删除、维护，存储的新建，删除，编辑

#### 4.11.3.3    预警

CPU、MEM、POD（cluster级别），CPU、MEM、系统盘（node级别），CPU、MEM（namespaces级别），资源使用超过80%预警阈值支持设置。Zcloud新加告警页面，展示所有告警，支持设置邮件告警。
